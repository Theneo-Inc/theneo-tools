...
- Each section's `index.md` starts with a `<!-- tab:slug -->` marker matching a declared tab: missing or unknown slug is an `error`; present but not at the very top is a `warning` (reported with a line number).
